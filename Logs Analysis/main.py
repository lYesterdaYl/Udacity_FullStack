...
class Report:
    ...
    def most_popular_article(self):
        ...
        cur.execute('''select ar.title, count(l."path") from log l LEFT JOIN articles ar on l."path" = concat('/article/',ar.slug) WHERE l.status != '404 NOT FOUND' and l."path" != '/' GROUP BY ar.title, l."path" ORDER BY count("path") DESC LIMIT 3''')
        rows = cur.fetchall()
    ...
    def most_popular_article_author(self):
        ...
        cur.execute('''select au.name, count(l."path") from log l LEFT JOIN articles ar on l."path" = concat('/article/',ar.slug) LEFT JOIN authors au on ar.author = au."id" WHERE l.status != '404 NOT FOUND' and l."path" != '/' GROUP BY au.name ORDER BY count(au.name) DESC''')
        rows = cur.fetchall()
